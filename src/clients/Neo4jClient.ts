...
export class Neo4jClient {
  ...
  constructor(uri: string, username: string, password: string, openAiClient: OpenAiClient) {
  ...
  async addNode(
    ...
    if (!properties.embedding) {
      ...
      properties.embedding = await this.openAiClient.createEmbedding(textForEmbedding);
    }
    const cypher = `
  ...
  async updateNode(nodeId: string, properties: Record<string, any>): Promise<Record<string, any>> {
    if (properties.content && !properties.embedding) {
      properties.embedding = await this.openAiClient.createEmbedding(properties.content);
    }
    const cypher = `
      MATCH (n)
      WHERE id(n) = $nodeId
  ...
  async performVectorSearch(
    ...
    const queryVector = await this.openAiClient.createEmbedding(queryContent);
  ...
  > {
    ...
    const directionClause = direction === 'INCOMING' ? '<-[r]-' : direction === 'OUTGOING' ? '-[r]->' : '-[r]-';
  ...
  async batchAddNodes(label: string, nodesList: Record<string, any>[]): Promise<Record<string, any>[]> {
    for (const node of nodesList) {
      if (node.content && !node.embedding) {
        node.embedding = await this.openAiClient.createEmbedding(node.content);
      }
    }
    const cypher = `
  ...
  async queryByRelationship(
    ...
    relationType: string,
    toType: string,
  ): Promise<Array<{from: Record<string, any>; to: Record<string, any>; relationship: Record<string, any>}>> {
    const cypher = `
